...
router.get('/jobs/:id/questions/:idx/edit',checkLoggedIn, checkAdmin, async (req,res) =>{/* idx here is the position of question in questions array */
    try {
       const job = await Job.findById(req.params.id);
       if (req.params.idx >= job.questions.length) {
        req.flash('error', 'out of range');
        res.redirect(`/jobs/${req.params.id}/questions`);
       }
